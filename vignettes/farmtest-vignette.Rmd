...
The observed data $X$ is assumed to follow a factor model $X = \mu + Bf + u$, where $f$ are the underlying factors, $B$ are the factors lodings, $u$ are the errrors, and $\mu$ is the mean effect to be tested. We assume the data is of dimension $p$ and the smaple size is $n$, leading to $p$ hypothesis tests.
